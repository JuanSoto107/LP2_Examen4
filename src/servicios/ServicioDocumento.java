...
public class ServicioDocumento {
    ...
    public static int busquedaBinaria(int i, int d, String s, int criterio) {
        ...
        if(criterio == 0) {
            n = documentos.get(medio).getApellido1();
        } else {
            n = documentos.get(medio).getDocumento();
        }

        if (s.compareTo(n) < 0) {
            return busquedaBinaria(i, medio - 1, s, criterio);

        } else if (s.compareTo(n) > 0) {
            return busquedaBinaria(medio + 1, d, s, criterio);
    ...
    public static int Anterior(int b, String s, int criterio) {

        for(int i = b - 1; i >= 0; i--) {
            if(criterio == 0) {
                if(s.equals(documentos.get(i).getApellido1())) {
                    b = i;
                    break;
                }
            } if(criterio == 1) {
                if(s.equals(documentos.get(i).getDocumento())) {
                    b = i;
                    break;
                }
            } 
    ...
    public static int Siguiente(int g, String s, int criterio) {

        for(int i = g + 1; i < documentos.size(); i++) {
            if(criterio == 0) {
                if(s.equals(documentos.get(i).getApellido1())) {
                    g = i;
                    break;
